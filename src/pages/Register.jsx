  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      await setDoc(doc(db, "clients", user.uid), {
        nom: form.nom, prenom: form.prenom, email: form.email, telephone: form.telephone, createdAt: new Date(),
      });
      navigate("/");
    } catch (err) {
      if (err.code === "auth/email-already-in-use") setError("Cet email est deja utilise.");
      else setError("Erreur lors de l'inscription. Reessayez.");
    }
    setLoading(false);
  }
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-12">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Creez votre compte</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input name="password" type={showPassword ? "text" : "password"} value={form.password} onChange={handleChange} required
                className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-gray-400 pr-12"
                placeholder="••••••••" />
          ...
          <div>
            ...
            <div className="relative">
              <input name="confirm" type={showPassword ? "text" : "password"} value={form.confirm} onChange={handleChange} required
                className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-gray-400 pr-12"
                placeholder="••••••••" />
              <button type="button" onClick={() => setShowPassword(!showPassword)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-lg">
