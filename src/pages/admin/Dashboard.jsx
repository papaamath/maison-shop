...
export default function Dashboard() {
  ...
  const tauxConversion = commandes.length > 0
    ? Math.round((livrees / commandes.length) * 100)
    : 0;

  // Données graphique évolution CA par jour
  const caParJour = {};
  commandes.forEach(cmd => {
    if (!cmd.createdAt?.seconds || cmd.statut === "Annulé") return;
    const date = new Date(cmd.createdAt.seconds * 1000).toLocaleDateString("fr-SN", {
  ...
  // Données graphique commandes par statut
  const dataStatuts = [
    { name: "En attente", value: enAttente },
    { name: "Confirmé", value: commandes.filter(c => c.statut === "Confirmé").length },
    { name: "En livraison", value: commandes.filter(c => c.statut === "En livraison").length },
    { name: "Livré", value: livrees },
    { name: "Annulé", value: annulees },
  ].filter(d => d.value > 0);

  // Top produits vendus
  const ventesParProduit = {};
  commandes.forEach(cmd => {
    if (cmd.statut === "Annulé") return;
  ...
  // Produits en rupture
  const ruptures = produits.filter(p => Number(p.stock || p.Stock || 0) === 0);
  const stockFaible = produits.filter(p => {
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              {[
                { label: "Chiffre d'affaires", value: formatPrix(ca), icon: "💰", color: "text-green-600" },
                { label: "Commandes totales", value: commandes.length, icon: "🛒", color: "" },
                { label: "En attente", value: enAttente, icon: "⏳", color: enAttente > 0 ? "text-yellow-600" : "" },
                { label: "Taux de livraison", value: `${tauxConversion}%`, icon: "🚚", color: "text-blue-600" },
              ].map(s => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                {ruptures.length > 0 && (
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                    <p className="font-bold text-red-700 mb-2">⚠️ Ruptures de stock ({ruptures.length})</p>
                    {ruptures.map(p => (
                  ...
                  <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                    ...
                    {stockFaible.map(p => (
                      <p key={p.id} className="text-sm text-yellow-600">
                        • {p.nom || p.Nom} — {p.stock || p.Stock} restant(s)
                      </p>
                    ))}
                  </div>
                )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                <h3 className="font-bold text-lg mb-4">📈 Évolution du chiffre d'affaires</h3>
                {dataCA.length === 0 ? (
                ...
                {dataStatuts.length === 0 ? (
                  ...
                  <div className="flex items-center gap-4">
                    <ResponsiveContainer width="60%" height={200}>
                      <PieChart>
                        <Pie data={dataStatuts} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80}>
                          {dataStatuts.map((_, i) => (
                ...
                {topProduits.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={topProduits} layout="vertical">
                      ...
                      <YAxis dataKey="nom" type="category" tick={{ fontSize: 10 }} width={100} />
                      <Tooltip />
