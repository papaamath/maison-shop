...
async function genererFacturePDF(cmd) {
  ...
  const lignes = (cmd.articles || []).map((a) => [
    clean(a.nom),
    String(a.quantite || 1),
    formatPrixPDF(a.prix),
    formatPrixPDF(Number(a.prix || 0) * Number(a.quantite || 1)),
  ]);
  ...
  const sousTotal =
    Number(cmd.total || 0) -
    Number(cmd.livraison || 0) +
    Number(cmd.reduction || 0);
  ...
  if (Number(cmd.reduction || 0) > 0) {
    doc.setTextColor(...vert);
    doc.text(`Reduction (${clean(cmd.codePromo || "")}) :`, xLabel, y);
    doc.text(`- ${formatPrixPDF(cmd.reduction)}`, xValue, y, {
      align: "right",
    });
    y += 7;
  }
  ...
  doc.text(formatPrixPDF(cmd.total), xValue, y + 8, { align: "right" });
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text("Merci pour votre confiance !", 105, pageHeight - 15, {
    align: "center",
  });
  doc.text(
    "B2S-STORE  |  Mbed Fass Yeumbeul, Dakar  |  +221 76 873 07 31",
    105,
    pageHeight - 9,
    { align: "center" }
  );
  ...
  async function handleRecherche(e) {
    ...
    try {
      ...
      const filtrees = toutes.filter((cmd) => {
        ...
        return (
          cmd.client?.telephone?.replace(/\s/g, "") ===
          recherche.replace(/\s/g, "")
        );
      });
